...
static constexpr std::array<FormatMapping, static_cast<size_t>(Format::COUNT)> c_FormatMap = { {
...
vk::Format Device::convertFormat(Format format)
{
    ...
    return static_cast<vk::Format>(c_FormatMap[static_cast<uint32_t>(format)].vkFormat);
}
...
static constexpr vk::SamplerAddressMode convertSamplerAddressMode(SamplerAddressMode mode)
{
    switch (mode)
    {
    ...
    default:
        return static_cast<vk::SamplerAddressMode>(0);
    }
}
...
static constexpr vk::ImageType pickImageType(uint32_t dimension)
{
    switch (dimension)
    {
    default:
...
static constexpr bool hasDepth(vk::Format format)
{
    return Device::guessImageAspectFlags(format, false) == vk::ImageAspectFlagBits::eDepth;
}

static constexpr vk::ImageUsageFlags pickImageUsage(const TextureDesc& desc, vk::Format format)
{
...
void Device::populateTexture(const std::shared_ptr<Texture>& texture, const TextureDesc& desc) const
{
    ...
    const vk::Format format = convertFormat(desc.format);
...
void Texture::setName(const std::string& debugName)
{
    ...
    auto raw = static_cast<VkImage>(handle);
...
vk::ImageView Texture::view(vk::ImageSubresourceRange subresource)
{
    ...
    const auto iterPair = m_views.emplace(subresource, m_context.device.createImageViewUnique(createInfo));
    return std::get<0>(iterPair)->second.get();
}
