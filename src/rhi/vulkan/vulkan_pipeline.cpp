...
{
static std::array<std::set<std::string_view>, 5> c_VertexAttrMap = { { { "inPos", "in.var.POSITION" },
                                                                       { "inTex", "inUV", "in.var.TEXCOORD0" },
                                                                       { "inNormal", "in.var.NORMAL0" },
                                                                       { "inTangent", "in.var.TANGENT0" },
                                                                       { "inColor" } } };

static constexpr uint32_t guessVertexInputBinding(const char* name)
{
...
static constexpr vk::PrimitiveTopology convertTopology(const PrimitiveType primitive)
{
    switch (primitive)
    {
...
ShaderPtr Device::createShader(const ShaderModule& shaderModule) const
{
    ...
    if (module.shader_stage == SPV_REFLECT_SHADER_STAGE_VERTEX_BIT)
    {
        ...
        for (SpvReflectInterfaceVariable* in : inputs)
        {
        ...
        std::ranges::sort(shader->attributeDesc,
...
{
    // DEPRECATED: REFLECTION DESCRIPTOR SET
    // This Pipeline is replaced with FULL BINDLESS!
    ...
    for (auto& set : sets)
    {
        ...
        std::initializer_list<vk::DescriptorType> types = { vk::DescriptorType::eSampledImage,
...
void BasePipeline::createDescriptorSet(uint32_t set)
{
    ...
    const vk::Result res = m_context.device.allocateDescriptorSets(&descriptorSetAllocInfo, &descriptorSet);
...
std::optional<vk::DescriptorType> BasePipeline::findBindingType(uint32_t set, uint32_t binding)
{
    if (descriptorAllocMap.contains(set))
    {
        const DescriptorAllocator& descriptorDesc = descriptorAllocMap[set];
...
void BasePipeline::updateSampler(uint32_t descriptor, uint32_t binding, SamplerPtr& sampler,
...
{
    const auto* native = checked_cast<Buffer*>(buffer.get());
...
{
    stages.emplace_back(vk::PipelineShaderStageCreateFlags(), shader->stageFlagBits, shader->shaderModule.get(),
...
rhi::PipelinePtr Device::createGraphicsPipeline(const std::span<ShaderModule>& shaderModules, const PipelineDesc& desc)
{
    ...
    // TOPOLOGY STATE
    vk::PipelineInputAssemblyStateCreateInfo pia(vk::PipelineInputAssemblyStateCreateFlags(),
                                                 convertTopology(desc.topology));

    // VIEWPORT STATE
    vk::Extent2D extent(desc.viewport.width, desc.viewport.height);
    vk::Viewport viewport(0.f, 0.f, static_cast<float>(extent.width), static_cast<float>(extent.height), 0.f, 1.0f);
    ...
    vk::PipelineMultisampleStateCreateInfo pm(vk::PipelineMultisampleStateCreateFlags(),
                                              pickImageSample(desc.sampleCount));
    ...
    // BLEND STATE
    // TODO: add parameters to enable blend
    ...
    vk::ResultValue<vk::UniquePipeline> res =
        m_context.device.createGraphicsPipelineUnique(m_context.pipelineCache, pipelineInfo);
...
rhi::PipelinePtr Device::createComputePipeline(const ShaderModule& shaderModule)
{
    ...
    const std::span shaders{ &shader, 1 };
    pipeline->reflectPipelineLayout(m_context.device, shaders);
    ...
    vk::ResultValue<vk::UniquePipeline> res =
        m_context.device.createComputePipelineUnique(m_context.pipelineCache, pipelineInfo);
