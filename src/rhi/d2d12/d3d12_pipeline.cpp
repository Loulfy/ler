...
{
    static std::string SitToString(D3D_SHADER_INPUT_TYPE type)
    {
        switch(type)
        {
    ...
    static char SitToChar(D3D12_DESCRIPTOR_RANGE_TYPE type)
    {
        switch(type)
        {
        default:
    ...
    static D3D12_DESCRIPTOR_RANGE_TYPE convertDescriptorType(D3D_SHADER_INPUT_TYPE type)
    {
        switch(type)
        {
        case D3D_SIT_CBUFFER:
    ...
    {
        uint32_t count;
        auto& root = m_descriptors.emplace_back();
    ...
    void Pipeline::updateSampler(uint32_t descriptor, uint32_t binding, SamplerPtr& sampler, const std::span<TexturePtr>& textures)
    {
        auto* native = checked_cast<Sampler*>(sampler.get());
        ...
        for(size_t i = 0; i < textures.size(); ++i)
        {
            ...
            m_context.device->CreateShaderResourceView(image->handle, &srvDesc, descriptorSet.tables[0].getCpuHandle(binding+i));
        }
    }
    ...
    void Pipeline::updateStorage(uint32_t descriptor, uint32_t binding, BufferPtr& buffer, uint64_t byteSize)
    {
        ...
        else
        {
            ...
            if(native->desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS && byteSize == 256)
            {
                ...
                m_context.device->CopyDescriptorsSimple(1, descriptorSet.tables[0].getCpuHandle(binding), cpuHandle, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
                //m_context.device->CreateUnorderedAccessView(native->handle, nullptr, &uavDesc, descriptorSet.tables[0].getCpuHandle(binding));
                //m_context.device->CopyDescriptorsSimple(1, native->clearCpuHandle.getCpuHandle(), descriptorSet.tables[0].getCpuHandle(binding), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
                //m_context.device->CopyDescriptorsSimple(1, native->clearGpuHandle.getCpuHandle(), descriptorSet.tables[0].getCpuHandle(binding), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
            }
            else
            {
                D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
