...
namespace ler::rhi::d3d12
{
    SwapChainPtr Device::createSwapChain(GLFWwindow* window, bool vsync)
    {
        ...
        m_factory->CreateSwapChainForHwnd(
            m_context.queue, // Swap chain needs the queue so that it can force a flush on it.
    ...
    void SwapChain::resize(uint32_t width, uint32_t height, bool vsync)
    {
        ...
        HRESULT hr = handle->ResizeBuffers(FrameCount, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING);
        if(FAILED(hr))
        ...
        // Create frame resources.
        {
            ...
            for (UINT n = 0; n < SwapChain::FrameCount; ++n)
            {
                ...
                m_context.device->CreateRenderTargetView(renderTargets[n].Get(), nullptr, texture->rtvDescriptor.getCpuHandle());
                m_images[n] = texture;
            }
        }
        ...
        log::info("SwapChain: Images({}), Extent({}x{}), Format({})",
            SwapChain::FrameCount,
            width, height, to_string(mapping.abstractFormat)
        );
    }
