...
static constexpr std::array<DescriptorPool, D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES> c_DescriptorPoolSize = { {
    { D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 64 },
    { D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, 8 },
...
class DescriptorHeapAllocation
{
  public:
    ...
    DescriptorHeapAllocation(DescriptorHeapAllocator* pAllocator, ID3D12DescriptorHeap* pHeap,
                             D3D12_CPU_DESCRIPTOR_HANDLE CpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE GpuHandle,
                             uint32_t descriptorSize, uint32_t NHandles) noexcept;
...
struct Buffer : public IBuffer
{
    ...
    [[nodiscard]] uint64_t sizeInBytes() const override { return desc.Width; }
...
struct Texture : public ITexture
{
    ...
    [[nodiscard]] Extent extent() const override { return {static_cast<uint32_t>(desc.Width), desc.Height}; }
    // clang-format on
};
...
class PSOLibrary
{
  public:
    ...
    PipelinePtr loadPipeline(const std::string& name, const std::span<ShaderModule>& shaderModules,
...
class Device : public IDevice
{
  public:
    ...
    [[nodiscard]] PipelinePtr loadPipeline(const std::string& name, const std::span<ShaderModule>& shaderModules,
                                           const PipelineDesc& desc) override;
    ...
    static void fillGraphicsPsoDesc(const PipelineDesc& desc, const std::span<ShaderPtr>& shaders,
